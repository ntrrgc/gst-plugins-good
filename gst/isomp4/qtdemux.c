    ...
    guint64 moof_offset, QtDemuxStream * stream)
{
  ...
  while (traf_node) {
    ...
    if (stream) {
      GstClockTime decode_time_ns =
          QTSTREAMTIME_TO_GSTTIME (stream, decode_time);
      if (decode_time_ns < min_decode_time_ns)
        min_decode_time_ns = decode_time_ns;
    }
  ...
  if (!qtdemux->upstream_format_is_time && !qtdemux->first_moof_already_parsed
      && !qtdemux->received_seek && GST_CLOCK_TIME_IS_VALID (min_decode_time_ns)
      && min_decode_time_ns != 0) {
    ...
    GST_DEBUG_OBJECT (qtdemux, "First fragment has a non-zero tfdt, "
        ...
        GST_TIME_ARGS (min_decode_time_ns));

    qtdemux->segment.start = min_decode_time_ns;
    qtdemux->segment.time = qtdemux->segment.position = min_decode_time_ns;

    for (iter = qtdemux->active_streams; iter; iter = g_list_next (iter)) {
      QtDemuxStream *stream = QTDEMUX_STREAM (iter->data);
      stream->time_position = min_decode_time_ns;
    }
    ...
    GstClockTime * _stop)
{
  ...
  stream->segment.position =
      stream->segment.rate > 0 ? stream->segment.start : stream->segment.stop;
