...
static void
write_free_box (guint8 * dst_mem, int size)
{
  g_assert (size >= 8);
    ...
    guint64 duration)
{
  ...
  event = &list->events[list->entry_count++];
...
/* If any of these behaviors is fixed or changed, update the constant before
 * running the tests. */

/* https://bugzilla.gnome.org/show_bug.cgi?id=794906
 *
 * In pull mode (the only mode where complex edit lists are supported),
...
/* Empty edits generate unnecessary GstSegments (preceding no frames).
 * Their start and stop properties refer to stream time, not buffer time. */
    ...
    MovieTemplate * movie_template)
{
  ...
  event_list_add_segment (&expected_events, typical_segment (&segment, 0,
          EXPECT_CLIPPING_TO_MOVIE_DURATION ? 2000000000 : 2333333333));
